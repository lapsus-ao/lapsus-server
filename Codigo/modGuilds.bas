...
Public vecClan() As Long
Public vecChuts() As Long
...
'CHOTS | Cantidad Maxima de Miembros
Public Const MAXMIEMBROS As Byte = 15
...
Dim GI          As Integer
    ...
    If EstaEnCastillo(Expulsador) Then
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "Z75")
        Exit Function
    End If
    
    'CHOTS | Guerras
    If UserList(Expulsador).guerra.enGuerra = True Then
        Call SendData(SendTarget.ToIndex, UserIndex, 0, ServerPackages.dialogo & "No puedes expulsar a alguien de tu clan en una guerra! Si deseas abandonar a tu equipo, tipea /SALIRGUERRA" & FONTTYPE_GUERRA)
        Exit Function
    End If
    ...
    If UserIndex > 0 Then
        ...
        If EstaEnCastillo(UserIndex) Then
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "Z74")
            Exit Function
        End If
        ...
        If GI > 0 Then
            If m_PuedeSalirDeClan(Expulsado, GI, Expulsador) Then
                ...
                Call Guilds(GI).DescontarGuildPoints(1)
...
Dim DummyString     As String
    ...
    If CANTIDADDECLANES < UBound(Guilds) Then
        ...
        UserList(FundadorIndex).flags.YaFundoClan = 1
...
Dim i               As Integer
    ...
    If Guilds(GuildIndex).EleccionesAbiertas Then
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "VOT")
    End If
...
Public Function PuedeFundarUnClan(ByVal UserIndex As Integer, ByVal Alineacion As ALINEACION_GUILD, ByRef refError As String) As Boolean
    ...
    If UserList(UserIndex).flags.YaFundoClan = 1 Then
        refError = "Ya Habías fundando un clan anteriormente!"
        Exit Function
    End If
    
    If UserList(UserIndex).Stats.ELV < 36 Or UserList(UserIndex).Stats.UserSkills(eSkill.Liderazgo) < 100 Then
        refError = "Para fundar un clan debes ser nivel 36 y tener 100 en liderazgo."
        Exit Function
    End If
...
Dim GI As Integer
    ...
    If FileExist(CharPath & Personaje & ".chr") Then
        ...
        'CHOTS | Si tiene clan no puede entrar
        GI = val(GetVar(CharPath & Personaje & ".chr", "GUILD", "GUILDINDEX"))
...
On Error GoTo errh
    Call SendData(SendTarget.ToAll, 0, 0, ServerPackages.dialogo & "Servidor> Revisando elecciones" & FONTTYPE_SERVER)
    For i = 1 To CANTIDADDECLANES
        If Not Guilds(i) Is Nothing Then
            If Guilds(i).RevisarElecciones Then
                Call SendData(SendTarget.ToAll, 0, 0, ServerPackages.dialogo & "        > " & Guilds(i).GetLeader & " es el nuevo lider de " & Guilds(i).GuildName & "!" & FONTTYPE_SERVER)
            End If
        End If
proximo:
    Next i
    Call SendData(SendTarget.ToAll, 0, 0, ServerPackages.dialogo & "Servidor> Elecciones revisadas" & FONTTYPE_SERVER)
Exit Sub
errh:
...
Dim Temps   As String
    ...
    End If
End Function
Public Function CHOTSGuildIndex(ByVal GuildName As String) As Integer
'CHOTS | Utiliza ByVal en vez de ByRef
Dim i As Integer
...
Dim i As Integer
    ...
    If Len(m_ListaDeMiembrosOnline) > 0 Then
        m_ListaDeMiembrosOnline = Left$(m_ListaDeMiembrosOnline, Len(m_ListaDeMiembrosOnline) - 2)
    End If
End Function

Public Function m_CantidadDeMiembrosOnline(ByVal UserIndex As Integer, ByVal GuildIndex As Integer) As Byte
Dim i As Integer
Dim contador As Byte
    ...
    If GuildIndex > 0 And GuildIndex <= CANTIDADDECLANES Then
        ...
        Wend
    End If
    m_CantidadDeMiembrosOnline = contador
End Function
...
Dim tInt As Integer
    ...
    For tInt = 1 To CANTIDADDECLANES
        tStr = tStr & Guilds(tInt).GuildName & "@" & Guilds(tInt).GetGuildPoints & ","
    Next tInt
    SendGuildsList = tStr
End Function
...
'GuildName = ReadField(1, GuildName, Asc("("))
'GuildName = Left(GuildName, Len(GuildName) - 1)
    ...
    tStr = tStr & CStr(Guilds(GI).GetGuildPoints) & "¬"
...
'For i = 1 To CANTIDADDECLANES
'    If Guilds(vecClan(i)).GetGuildPoints <> -1 Then
'        tStr = tStr & Guilds(vecClan(i)).GuildName & "@" & Guilds(vecClan(i)).GetGuildPoints & "¬"
'    End If
'Next i
...
'devuelve el guildindex
    ...
    If GI > 0 Then
        ...
        Call SendData(SendTarget.ToIndex, UserIndex, 0, ServerPackages.dialogo & "Conectado a : " & GuildName & FONTTYPE_GUILD)
        GMEscuchaClan = GI
        UserList(UserIndex).EscucheClan = GI
    Else
        Call SendData(SendTarget.ToIndex, UserIndex, 0, ServerPackages.dialogo & "Error, el clan no existe" & FONTTYPE_GUILD)
        GMEscuchaClan = 0
    End If
...
Dim NuevoGuildIndex     As Integer
    ...
    If Guilds(NuevoGuildIndex).CantidadDeMiembros >= MAXMIEMBROS Then
    ...
    If Guilds(NuevoGuildIndex).CantidadAspirantes + Guilds(NuevoGuildIndex).CantidadDeMiembros = MAXMIEMBROS Then
...
Dim AspiranteUI     As Integer
    ...
    Else
        If Not m_EstadoPermiteEntrarChar(Aspirante, GI) Then
            refError = Aspirante & " no puede entrar a un clan"
            Call Guilds(GI).RetirarAspirante(Aspirante, NroAspirante)
            Exit Function
        End If
    End If
