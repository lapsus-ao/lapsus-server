...
Function UserCompraObj(ByVal UserIndex As Integer, ByVal ObjIndex As Integer, ByVal NpcIndex As Integer, ByVal Cantidad As Integer, ByRef CHOTSlot As Byte) As Boolean
On Error GoTo errorh
    ...
    If ObjData(obji).Real = 1 Then
        If Npclist(NpcIndex).Name <> "SR" Then
            Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, ServerPackages.dialogo & vbWhite & "°" & "Lo siento, la ropa faccionaria solo es para muestra, no tengo autorización para venderla. Diríjete al sastre de tu ejército." & "°" & str(Npclist(NpcIndex).char.CharIndex))
            Exit Function
        End If
    End If
...
On Error GoTo errorh
    ...
    If ObjData(obji).Newbie = 1 Then
        Call SendData(SendTarget.ToIndex, UserIndex, 0, ServerPackages.dialogo & "No comercio objetos para newbies." & FONTTYPE_INFO)
        Exit Sub
    End If
    ...
    If Npclist(NpcIndex).TipoItems <> eOBJType.otCualquiera Then
        ...
        If Npclist(NpcIndex).TipoItems <> ObjData(obji).OBJType Then
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "Z49")
            Exit Sub
        End If
    End If
...
On Error GoTo errhandler
    ...
    If UserList(UserIndex).Stats.GLD >= (val * Cantidad) Then
        If Npclist(UserList(UserIndex).flags.TargetNPC).Invent.Object(i).Amount > 0 Then
            ...
            If Not UserCompraObj(UserIndex, CInt(i), UserList(UserIndex).flags.TargetNPC, Cantidad, CHOTSlot) Then
                Call SendData(SendTarget.ToIndex, UserIndex, 0, ServerPackages.dialogo & "No puedes comprar este ítem." & FONTTYPE_INFO)
            Else
        ...
        End If
    Else
        Call SendData(SendTarget.ToIndex, UserIndex, 0, ServerPackages.dialogo & "No tenes suficiente dinero." & FONTTYPE_INFO)
    End If
Exit Sub
...
On Error GoTo errhandler
    ...
    If ObjData(UserList(UserIndex).Invent.Object(item).ObjIndex).Real = 1 Then
        If Npclist(NpcIndex).Name <> "SR" Then
            Call SendData(SendTarget.ToIndex, UserIndex, 0, ServerPackages.dialogo & "Las armaduras faccionarias sólo las puedes vender a sus respectivos Sastres" & FONTTYPE_WARNING)
...
Sub EnviarNpcInv(ByVal UserIndex As Integer, ByVal NpcIndex As Integer)
    ...
    'SendData SendTarget.ToIndex, UserIndex, 0, "NPCÑ" 'CHOTS | Inicializa el inventario del NPC y luego envía solamente donde hay items
    ...
    For i = 1 To MAX_INVENTORY_SLOTS
        ...
        Else
            SendData SendTarget.ToIndex, UserIndex, 0, "NPCI" & "N" 'CHOTS | Optimizado
        End If
    Next i
End Sub
