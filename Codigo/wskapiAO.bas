...
TempKey = UserList(Slot).ClavePrivada
EncriptedString = ChotsEncrypt(str, Slot)
'CHOTS | Seguridad
...
If UserList(Slot).ConnID <> -1 And UserList(Slot).ConnIDValida Then
    If ((UserList(Slot).ColaSalida.Count = 0)) Or (Not Encolar) Then
        Ret = send(ByVal UserList(Slot).ConnID, ByVal EncriptedString, ByVal Len(EncriptedString), ByVal 0)
        If Ret < 0 Then
            UltError = Err.LastDllError
            If UltError = WSAEWOULDBLOCK Then
                If Encolar Then
                    ...
                    UserList(Slot).ClavePrivada = TempKey
                End If
            End If
            Retorno = UltError
        End If
    ...
    If NewIndex <= MaxUsers Then
        ...
        For i = 1 To BanIps.Count
            If BanIps.item(i) = UserList(NewIndex).ip Then
                ' Bysnack, CHOTS | Anti Bots, no envia mas el mensaje de IP Baneada
                Call WSApiCloseSocket(NuevoSock)
                Exit Sub
            End If
        Next i
        ...
        'CHOTS | Anti Bombardeo by RevivalAO
...
Datos = ChotsDecrypt(Datos, Slot) 'CHOTS | Seguridad
