...
Function ModificadorEvasion(ByVal Clase As String, Optional UserIndex As Integer) As Single
...
Select Case UCase$(Clase)
    ...
    Case "PALADIN"
        ModificadorEvasion = 0.9
    Case "ASESINO"
        If EstaLiberado(UserIndex) And (UCase$(UserList(UserIndex).Raza) = "ELFO" Or UCase$(UserList(UserIndex).Raza) = "GNOMO") Then
            ModificadorEvasion = 1.05 'CHOTS | Gemas (Ase Elfo, Ase Gnomo)
        Else
            ModificadorEvasion = 1
        End If
    Case "BARDO"
        If EstaLiberado(UserIndex) And UCase$(UserList(UserIndex).Raza) = "ELFO OSCURO" Then
            ModificadorEvasion = 1.1
        Else 'CHOTS | Gemas (Bardo EO)
            ModificadorEvasion = 1.05
        End If
    Case "MAGO"
        If EstaLiberado(UserIndex) And (UCase$(UserList(UserIndex).Raza) = "GNOMO" Or UCase$(UserList(UserIndex).Raza) = "ORCO") Then
            ModificadorEvasion = 0.7
        Else 'CHOTS | Gemas (Mago Gnomo, Mago Orco)
            ModificadorEvasion = 0.5
        End If
    Case "CLERIGO"
        If EstaLiberado(UserIndex) And UCase$(UserList(UserIndex).Raza) = "ORCO" Then
            ModificadorEvasion = 0.85
        Else 'CHOTS | Gemas (Clero Orco)
            ModificadorEvasion = 0.75
        End If
    Case "DRUIDA"
        If EstaLiberado(UserIndex) And (UCase$(UserList(UserIndex).Raza) = "ELFO OSCURO" Or UCase$(UserList(UserIndex).Raza) = "GNOMO") Then
            ModificadorEvasion = 0.8
        Else 'CHOTS | Gemas (Druida EO)
...
Function ModificadorPoderAtaqueArmas(ByVal Clase As String, Optional UserIndex As Integer) As Single

Select Case UCase$(Clase)
    Case "GUERRERO"
        If EstaLiberado(UserIndex) And UCase$(UserList(UserIndex).Raza) = "ELFO" Then
            ModificadorPoderAtaqueArmas = 1.2
        Else
    ...
    Case "PALADIN"
        If EstaLiberado(UserIndex) And (UCase$(UserList(UserIndex).Raza) = "ELFO" Or UCase$(UserList(UserIndex).Raza) = "GNOMO") Then
            ModificadorPoderAtaqueArmas = 1 'CHOTS | Gemas (Pala Elfo, Pala Gnomo)
        Else
    ...
    Case "ASESINO"
            ModificadorPoderAtaqueArmas = 0.9
    Case "CLERIGO"
        If EstaLiberado(UserIndex) And (UCase$(UserList(UserIndex).Raza) = "HUMANO" Or UCase$(UserList(UserIndex).Raza) = "ELFO" Or UCase$(UserList(UserIndex).Raza) = "GNOMO") Then
            ModificadorPoderAtaqueArmas = 0.85 'CHOTS | Gemas (Clero Elfo, Clero Gnomo, Clero Humano)
        Else
            ModificadorPoderAtaqueArmas = 0.8
        End If
    Case "BARDO"
...
Function ModificadorPoderAtaqueProyectiles(ByVal Clase As String, Optional UserIndex As Integer) As Single
    ...
    Case "CAZADOR"
        If EstaLiberado(UserIndex) Then
            If UCase$(UserList(UserIndex).Raza) = "ELFO" Then
                ModificadorPoderAtaqueProyectiles = 1.15 'CHOTS | Gemas (Caza Elfo)
            ElseIf UCase$(UserList(UserIndex).Raza) = "HUMANO" Then
                ModificadorPoderAtaqueProyectiles = 1.1 'CHOTS | Gemas (Caza Humano)
            Else
                ModificadorPoderAtaqueProyectiles = 1.05
            End If
        Else
            ModificadorPoderAtaqueProyectiles = 1.05
        End If
...
Function ModicadorDañoClaseArmas(ByVal Clase As String, Optional UserIndex As Integer) As Single
...
Select Case UCase$(Clase)
    ...
    Case "PALADIN"
        If EstaLiberado(UserIndex) And UCase$(UserList(UserIndex).Raza) = "ELFO OSCURO" Then
            ModicadorDañoClaseArmas = 1 'CHOTS | Gemas (Pala EO)
        Else
            ModicadorDañoClaseArmas = 0.9
        End If
    Case "ASESINO"
...
Function ModicadorDañoClaseProyectiles(ByVal Clase As String, Optional UserIndex As Integer) As Single
...
Select Case UCase$(Clase)
    ...
    Case "CAZADOR"
        If EstaLiberado(UserIndex) And UCase$(UserList(UserIndex).Raza) = "ELFO OSCURO" Then
            ModicadorDañoClaseProyectiles = 1.15 'CHOTS | Gemas (Caza EO)
        Else
...
Function ModEvasionDeEscudoClase(ByVal Clase As String, Optional UserIndex As Integer) As Single

Select Case UCase$(Clase)
    Case "GUERRERO"
        If EstaLiberado(UserIndex) And UCase$(UserList(UserIndex).Raza) = "ORCO" Then
            ModEvasionDeEscudoClase = 1.1 'CHOTS | Gemas (Guerre Orco)
        Else
            ModEvasionDeEscudoClase = 1
        End If
    Case "CAZADOR"
        If EstaLiberado(UserIndex) And UCase$(UserList(UserIndex).Raza) = "ORCO" Then
            ModEvasionDeEscudoClase = 1 'CHOTS | Gemas (Caza Orco)
        Else
...
PoderEvasionEscudo = (UserList(UserIndex).Stats.UserSkills(eSkill.Defensa) * ModEvasionDeEscudoClase(UserList(UserIndex).Clase, UserIndex)) / 2
    ...
    Dim lTemp As Long
     With UserList(UserIndex)
       lTemp = (.Stats.UserSkills(eSkill.Tacticas) + _
          ...
          ModificadorEvasion(.Clase, UserIndex)
...
If UserList(UserIndex).Stats.UserSkills(eSkill.Armas) < 31 Then
    ...
    ModificadorPoderAtaqueArmas(UserList(UserIndex).Clase, UserIndex))
...
If UserList(UserIndex).Stats.UserSkills(eSkill.Proyectiles) < 31 Then
    ...
    ModificadorPoderAtaqueProyectiles(UserList(UserIndex).Clase, UserIndex))
...
If UserList(UserIndex).Stats.UserSkills(eSkill.Wresterling) < 31 Then
    ...
    ModificadorPoderAtaqueArmas(UserIndex))
...
If UserList(UserIndex).Invent.WeaponEqpObjIndex > 0 Then
    ...
    If NpcIndex > 0 Then
        ...
        If UserList(UserIndex).Invent.WeaponEqpObjIndex = EspadaMataDragonesIndex Or UserList(UserIndex).Invent.WeaponEqpObjIndex = ExterminaIndex Then ' Usa la matadragones?
            ModifClase = ModicadorDañoClaseArmas(UserList(UserIndex).Clase, UserIndex)
        ...
        Else ' daño comun
           If Arma.proyectil = 1 Then
                ModifClase = ModicadorDañoClaseProyectiles(UserList(UserIndex).Clase, UserIndex)
                    DañoArma = RandomNumber(Arma.MinHIT, Arma.MaxHIT)
                DañoMaxArma = Arma.MaxHIT
    ...
    Else ' Ataca usuario
        If UserList(UserIndex).Invent.WeaponEqpObjIndex = EspadaMataDragonesIndex Or UserList(UserIndex).Invent.WeaponEqpObjIndex = ExterminaIndex Then
            ModifClase = ModicadorDañoClaseArmas(UserList(UserIndex).Clase, UserIndex)
...
If Npclist(NpcIndex).NPCtype = eNPCType.reydelcastillo Then
    ...
    If UserList(UserIndex).Pos.Map = CastilloOeste And CASTILLO_OESTE_DUEÑO = Guilds(UserList(UserIndex).GuildIndex).GuildName Or _
        ...
        UserList(UserIndex).Pos.Map = CastilloFortaleza Then Exit Function
    End If
'CHOTS | Sistema de Castillos

'CHOTS | Guerras
If Npclist(NpcIndex).guerra.enGuerra = True And UserList(UserIndex).guerra.enGuerra = True And Npclist(NpcIndex).Movement = TipoAI.ESTATICO Then
    If Npclist(NpcIndex).guerra.team = UserList(UserIndex).guerra.team Then Exit Function
End If
'CHOTS | Guerras
...
If NpcImpacto(NpcIndex, UserIndex) Then
    ...
    If UserList(UserIndex).flags.Meditando = False Then
        If UserList(UserIndex).flags.Navegando = 0 Then Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "CXF" & UserList(UserIndex).char.CharIndex & "," & FXSANGRE & "," & 0)
    End If
...
If UserList(UserIndex).flags.Seguro And Npclist(NpcIndex).MaestroUser <> 0 Then
    If Not Criminal(Npclist(NpcIndex).MaestroUser) Then
        Call SendData(SendTarget.ToIndex, UserIndex, 0, ServerPackages.dialogo & "Debes sacar el seguro antes de poder atacar una mascota de un ciudadano." & FONTTYPE_WARNING)
        Exit Sub
    End If
End If
...
If UserList(UserIndex).Faccion.ArmadaReal = 1 And Npclist(NpcIndex).MaestroUser <> 0 Then
    If Not Criminal(Npclist(NpcIndex).MaestroUser) Then
        Call SendData(SendTarget.ToIndex, UserIndex, 0, ServerPackages.dialogo & "Los soldados del Ejercito Real tienen prohibido atacar ciudadanos y sus macotas." & FONTTYPE_WARNING)
        Exit Sub
    End If
End If
...
If Npclist(NpcIndex).NPCtype = eNPCType.GuardiaReal And UserList(UserIndex).flags.Seguro Then
    Call SendData(SendTarget.ToIndex, UserIndex, 0, ServerPackages.dialogo & "Debes quitar el seguro para atacar guardias." & FONTTYPE_FIGHT)
    Exit Sub
End If
...
If IntervaloPermiteAtacar(UserIndex) Then
    ...
    If MapData(AttackPos.Map, AttackPos.X, AttackPos.Y).NpcIndex > 0 Then
        ...
        If Npclist(MapData(AttackPos.Map, AttackPos.X, AttackPos.Y).NpcIndex).Attackable Then
            ...
            If Npclist(MapData(AttackPos.Map, AttackPos.X, AttackPos.Y).NpcIndex).MaestroUser > 0 And _
               MapInfo(Npclist(MapData(AttackPos.Map, AttackPos.X, AttackPos.Y).NpcIndex).Pos.Map).Pk = False Then
                    Call SendData(SendTarget.ToIndex, UserIndex, 0, ServerPackages.dialogo & "No podés atacar mascotas en zonas seguras" & FONTTYPE_FIGHT)
                    Exit Sub
            End If
        ...
        Else
            Call SendData(SendTarget.ToIndex, UserIndex, 0, ServerPackages.dialogo & "No podés atacar a este NPC" & FONTTYPE_FIGHT)
        End If
...
If Distancia(UserList(AtacanteIndex).Pos, UserList(VictimaIndex).Pos) > MAXDISTANCIAARCO Then
   Call SendData(SendTarget.ToIndex, AtacanteIndex, 0, "Z27")
   Exit Sub
End If
...
If UsuarioImpacto(AtacanteIndex, VictimaIndex) Then
    ...
    If UserList(VictimaIndex).flags.Navegando = 0 Then Call SendData(SendTarget.ToPCArea, VictimaIndex, UserList(VictimaIndex).Pos.Map, "CXF" & UserList(VictimaIndex).char.CharIndex & "," & FXSANGRE & "," & 0)
...
'CHOTS | Monturas
If UserList(AtacanteIndex).flags.montandoMascota = 1 Then
    Obj = ObjData(UserList(AtacanteIndex).Invent.MonturaObjIndex)
    daño = daño + RandomNumber(Obj.MinAta, Obj.MaxAta)
End If
...
Select Case Lugar
    ...
    Case Else
        ...
        'CHOTS | Monturas
        If UserList(VictimaIndex).flags.montandoMascota = 1 Then
            Obj = ObjData(UserList(VictimaIndex).Invent.MonturaObjIndex)
            daño = daño - RandomNumber(Obj.MinDefensa, Obj.MaxDefensa)
        End If
...
Sub UsuarioAtacadoPorUsuario(ByVal AttackerIndex As Integer, ByVal VictimIndex As Integer)
    ...
    If UserList(AttackerIndex).flags.enTorneoAuto Then Exit Sub
    If UserList(AttackerIndex).guerra.enGuerra Then Exit Sub
    If EstaEnCastillo(AttackerIndex) And UserList(AttackerIndex).Faccion.ArmadaReal = 1 Then Exit Sub
...
If UserList(VictimIndex).flags.Muerto = 1 Then
    SendData SendTarget.ToIndex, AttackerIndex, 0, ServerPackages.dialogo & "No podes atacar a un espiritu" & FONTTYPE_INFO
    PuedeAtacar = False
    Exit Function
End If
...
If UserList(VictimIndex).flags.Privilegios >= PlayerType.Consejero Then
    PuedeAtacar = False
    If UserList(VictimIndex).flags.AdminInvisible = 0 Then SendData SendTarget.ToIndex, AttackerIndex, 0, ServerPackages.dialogo & "¡¡No podes atacar a los administradores del juego!! " & FONTTYPE_WARNING
    Exit Function
End If

If MapInfo(UserList(VictimIndex).Pos.Map).Pk = False Then
    Call SendData(SendTarget.ToIndex, AttackerIndex, 0, ServerPackages.dialogo & "Esta es una zona segura, aqui no podes atacar otros usuarios." & FONTTYPE_WARNING)
    PuedeAtacar = False
    Exit Function
End If
...
If MapData(UserList(VictimIndex).Pos.Map, UserList(VictimIndex).Pos.X, UserList(VictimIndex).Pos.Y).trigger = eTrigger.ZONASEGURA Or _
    ...
    Call SendData(SendTarget.ToIndex, AttackerIndex, 0, ServerPackages.dialogo & "No puedes pelear aqui." & FONTTYPE_WARNING)
    PuedeAtacar = False
    Exit Function
End If
...
If UserList(AttackerIndex).flags.SeguroClan = True Then
    If UserList(AttackerIndex).GuildIndex > 0 And UserList(VictimIndex).GuildIndex > 0 Then
        If UserList(VictimIndex).GuildIndex = UserList(AttackerIndex).GuildIndex Then
            Call SendData(SendTarget.ToIndex, AttackerIndex, 0, "Z66")
            PuedeAtacar = False
            Exit Function
        End If
    End If
...
'CHOTS | Armadas se atacan en Castillo y torneo Auto
If EstaEnCastillo(AttackerIndex) Or UserList(AttackerIndex).flags.enTorneoAuto Or UserList(AttackerIndex).guerra.enGuerra Then
    PuedeAtacar = True
    Exit Function
End If
'CHOTS | Armadas se atacan en Castillo y torneo Auto
...
If UserList(AttackerIndex).flags.Seguro Then
    If Not Criminal(VictimIndex) Then
        Call SendData(SendTarget.ToIndex, AttackerIndex, 0, ServerPackages.dialogo & "No podes atacar ciudadanos, con el Seguro Activado!" & FONTTYPE_FIGHT)
        PuedeAtacar = False
        Exit Function
    End If
End If

If UserList(AttackerIndex).flags.SeguroCaos = True And UserList(VictimIndex).Faccion.FuerzasCaos = 1 Then
    Call SendData(SendTarget.ToIndex, AttackerIndex, 0, "Z70")
    PuedeAtacar = False
    Exit Function
End If

If (Not Criminal(VictimIndex)) And (UserList(AttackerIndex).Faccion.ArmadaReal = 1) Then
    Call SendData(SendTarget.ToIndex, AttackerIndex, 0, ServerPackages.dialogo & "Los soldados del Ejercito Real tienen prohibido atacar ciudadanos." & FONTTYPE_WARNING)
    PuedeAtacar = False
    Exit Function
End If
...
If Npclist(NpcIndex).MaestroUser > 0 Then
    If Not Criminal(AttackerIndex) And Not Criminal(Npclist(NpcIndex).MaestroUser) Then
        If UserList(AttackerIndex).flags.Seguro Then
            Call SendData(SendTarget.ToIndex, AttackerIndex, 0, ServerPackages.dialogo & "Para atacar mascotas de ciudadanos debes quitarte el seguro" & FONTTYPE_FIGHT)
...
If ExpaDar > 0 Then
    If UserList(UserIndex).PartyIndex > 0 Then
        Call mdParty.ObtenerExito(UserIndex, ExpaDar * MultExp, Npclist(NpcIndex).Pos.Map, Npclist(NpcIndex).Pos.X, Npclist(NpcIndex).Pos.Y)
    Else
        UserList(UserIndex).Stats.Exp = UserList(UserIndex).Stats.Exp + ExpaDar * MultExp
        If UserList(UserIndex).Stats.Exp > MAXEXP Then _
            UserList(UserIndex).Stats.Exp = MAXEXP
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "CHW" & (ExpaDar * MultExp))
    End If
    ...
    If ObjInd > 0 Then
        If (ObjData(ObjInd).Envenena = 1) Then
            ...
            If num < 60 Then
                ...
                Call SendData(SendTarget.ToIndex, VictimaIndex, 0, ServerPackages.dialogo & UserList(AtacanteIndex).Name & " te ha envenenado!!" & FONTTYPE_FIGHT)
                Call SendData(SendTarget.ToIndex, AtacanteIndex, 0, ServerPackages.dialogo & "Has envenenado a " & UserList(VictimaIndex).Name & "!!" & FONTTYPE_FIGHT)
            End If
        End If
    End If
End If
