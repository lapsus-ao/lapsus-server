    ...
    If Npclist(NpcIndex).flags.Inmovilizado = 0 Or Npclist(NpcIndex).char.Heading = headingloop Then
        ...
        If InMapBounds(nPos.Map, nPos.X, nPos.Y) Then
            ...
            If UI > 0 And Not atacoPJ Then
                If UserList(UI).flags.Muerto = 0 Then
                    If Npclist(NpcIndex).Movement = TipoAI.ESTATICO And Npclist(NpcIndex).guerra.enGuerra = True And UserList(UI).guerra.enGuerra = True Then
                        If Npclist(NpcIndex).guerra.team = UserList(UI).guerra.team Then Exit Sub
                    End If
                    atacoPJ = True
                    If CHOTS = 1 Then
                    ...
                    If UserList(UI).Name = Npclist(NpcIndex).flags.AttackedBy Then
                        If Npclist(NpcIndex).MaestroUser > 0 Then
                            If Not Criminal(Npclist(NpcIndex).MaestroUser) And Not Criminal(UI) And (UserList(Npclist(NpcIndex).MaestroUser).flags.Seguro Or UserList(Npclist(NpcIndex).MaestroUser).Faccion.ArmadaReal = 1) Then
                                Call SendData(SendTarget.ToIndex, Npclist(NpcIndex).MaestroUser, 0, ServerPackages.dialogo & "La mascota no atacará a ciudadanos si eres miembro de la Armada Real o tienes el seguro activado" & FONTTYPE_INFO)
            ...
            If X >= MinXBorder And X <= MaxXBorder And Y >= MinYBorder And Y <= MaxYBorder Then
               ...
               If NI > 0 Then
                    If Npclist(NpcIndex).TargetNPC = NI Then
                         ...
                         Else
                            'aca verificamosss la distancia de ataque
                            If Distancia(Npclist(NpcIndex).Pos, Npclist(NI).Pos) <= 3 Then 'CHOTS | Atacan de lejos los eles
                                Call SistemaCombate.NpcAtacaNpc(NpcIndex, NI)
                            End If
                         End If
                         If Npclist(NpcIndex).flags.Inmovilizado = 1 Then Exit Sub
...
On Error GoTo ErrorHandler
        ...
        Select Case Npclist(NpcIndex).Movement
            ...
            Case TipoAI.NpcMaloAtacaUsersBuenos, TipoAI.ESTATICO
...
For Y = Npclist(NpcIndex).Pos.Y - 8 To Npclist(NpcIndex).Pos.Y + 8    'Makes a loop that looks at
     For X = Npclist(NpcIndex).Pos.X - 8 To Npclist(NpcIndex).Pos.X + 8   '5 tiles in every direction
         ...
         If X > MinXBorder And X < MaxXBorder And Y > MinYBorder And Y < MaxYBorder Then
             ...
             If MapData(Npclist(NpcIndex).Pos.Map, X, Y).UserIndex > 0 Then
                If Npclist(NpcIndex).NPCtype = eNPCType.reydelcastillo Then
                    If UserList(MapData(Npclist(NpcIndex).Pos.Map, X, Y).UserIndex).Pos.Map = CastilloOeste And CASTILLO_OESTE_DUEÑO = Guilds(UserList(MapData(Npclist(NpcIndex).Pos.Map, X, Y).UserIndex).GuildIndex).GuildName Or _
                        UserList(MapData(Npclist(NpcIndex).Pos.Map, X, Y).UserIndex).Pos.Map = CastilloEste And CASTILLO_ESTE_DUEÑO = Guilds(UserList(MapData(Npclist(NpcIndex).Pos.Map, X, Y).UserIndex).GuildIndex).GuildName Or _
                        UserList(MapData(Npclist(NpcIndex).Pos.Map, X, Y).UserIndex).Pos.Map = CastilloNorte And CASTILLO_NORTE_DUEÑO = Guilds(UserList(MapData(Npclist(NpcIndex).Pos.Map, X, Y).UserIndex).GuildIndex).GuildName Or _
                        UserList(MapData(Npclist(NpcIndex).Pos.Map, X, Y).UserIndex).Pos.Map = CastilloSur And CASTILLO_SUR_DUEÑO = Guilds(UserList(MapData(Npclist(NpcIndex).Pos.Map, X, Y).UserIndex).GuildIndex).GuildName Or _
                         UserList(MapData(Npclist(NpcIndex).Pos.Map, X, Y).UserIndex).Pos.Map = CastilloFortaleza Then Exit Function
                End If
