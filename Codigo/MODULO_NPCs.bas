...
On Error GoTo errhandler
   ...
   MiNPC = Npclist(NpcIndex)
    ...
    'CHOTS | Guerras
    If UserIndex <> 0 And Npclist(NpcIndex).guerra.enGuerra = True Then
        Call MuereNpcGuerra(UserIndex, NpcIndex)
        Exit Sub
    End If
    'CHOTS | Guerras
   
    'CHOTS | Sistema de Castillos
    If UserIndex <> 0 And Npclist(NpcIndex).Numero = ReyNpcN Then
    ...
    'CHOTS | Invocaciones
    If UserIndex <> 0 And Npclist(NpcIndex).Numero = INVOCACION_NPC Then
        Call MuereInvocacion(NpcIndex)
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "Z50")
    ...
    'CHOTS | Quests
    If UserIndex <> 0 Then
        If UserList(UserIndex).Quest.nroQuest <> 0 And Npclist(NpcIndex).Numero = UserList(UserIndex).Quest.numNpc Then
            UserList(UserIndex).Quest.cantMatados = UserList(UserIndex).Quest.cantMatados + 1
        End If
    End If
    'CHOTS | Quests
   ...
   If UserIndex > 0 Then ' Lo mato un usuario?
        ...
        If MiNPC.flags.ExpCount > 0 Then
            ...
            Else
                ...
                Call SendData(SendTarget.ToIndex, UserIndex, 0, "CHW" & MiNPC.flags.ExpCount)
                'CHOTS | ENVIAREXP iba acá
            End If
            MiNPC.flags.ExpCount = 0
        Else
            Call SendData(SendTarget.ToIndex, UserIndex, 0, ServerPackages.dialogo & "No has ganado experiencia al matar la criatura." & FONTTYPE_FIGHT)
        End If
   ...
   If MiNPC.MaestroUser = 0 Then
        'Tiramos el oro
        If UserIndex <> 0 Then Call NPCTirarOro(MiNPC, UserIndex)
        If UserIndex <> 0 Then Call EnviarOro(UserIndex)
...
errhandler:
    Call LogError("Error en MuereNpc. Userindex: " & UserIndex & "Error: " & Err.number & " | " & Err.Description)
...
On Error GoTo errhandler
    ...
    'CHOTS | Monturas
    If Npclist(NpcIndex).Numero >= 586 And Npclist(NpcIndex).Numero <= 591 Then
        Call respawnMontura(Npclist(NpcIndex).Numero)
    End If
    'CHOTS | Monturas
...
End Sub
Sub CrearNPC2(NroNPC As Integer, mapa As Integer, OrigPos As WorldPos)
'Call LogTarea("Sub CrearNPC")
'Crea un NPC del tipo NRONPC
...
Dim Y As Integer
    ...
    Else
        ...
        Do While Not PosicionValida
            ...
            Call ClosestLegalPos2(Pos, newpos)  'Nos devuelve la posicion valida mas cercana
            If newpos.X <> 0 Then altpos.X = newpos.X
            If newpos.Y <> 0 Then altpos.Y = newpos.Y     'posicion alternativa (para evitar el anti respawn)
            'Si X e Y son iguales a 0 significa que no se encontro posicion valida
            If LegalPosNPC2(newpos.Map, newpos.X, newpos.Y, Npclist(nIndex).flags.AguaValida) And _
               Not HayPCarea(newpos) Then
                'Asignamos las nuevas coordenas solo si son validas
                Npclist(nIndex).Pos.Map = newpos.Map
                Npclist(nIndex).Pos.X = newpos.X
                Npclist(nIndex).Pos.Y = newpos.Y
            ...
            If Iteraciones > MAXSPAWNATTEMPS Then
                ...
                Else
                    altpos.X = 50
                    altpos.Y = 50
                    Call ClosestLegalPos2(altpos, newpos)
                    If newpos.X <> 0 And newpos.Y <> 0 Then
                        Npclist(nIndex).Pos.Map = newpos.Map
                        Npclist(nIndex).Pos.X = newpos.X
                        Npclist(nIndex).Pos.Y = newpos.Y
...
Dim CharIndex As Integer
    ...
    Else
        Call SendData(sndRoute, sndIndex, sndMap, ServerPackages.crearNpc & Npclist(NpcIndex).char.Body & "," & Npclist(NpcIndex).char.Head & "," & Npclist(NpcIndex).char.Heading & "," & Npclist(NpcIndex).char.CharIndex & "," & X & "," & Y)
    End If
...
If sndRoute = SendTarget.ToMap Then
    Call SendToNpcArea(NpcIndex, ServerPackages.borrarChar & code)
Else
...
#If SeguridadAlkon Then
            Call SendToNpcArea(NpcIndex, ServerPackages.moverNpc & Encriptacion.MoveNPCCrypt(NpcIndex, nPos.X, nPos.Y))
#Else
            Call SendToNpcArea(NpcIndex, ServerPackages.moverNpc & Npclist(NpcIndex).char.CharIndex & "," & nPos.X & "," & nPos.Y)
#End If
...
If n < 30 Then
    ...
    Call SendData(SendTarget.ToIndex, UserIndex, 0, ServerPackages.dialogo & "¡¡La criatura te ha envenenado!!" & FONTTYPE_FIGHT)
End If
...
Do While Not PosicionValida
        ...
        Call ClosestLegalPos(Pos, newpos)  'Nos devuelve la posicion valida mas cercana
        'Si X e Y son iguales a 0 significa que no se encontro posicion valida
        If Npclist(nIndex).flags.TierraInvalida = 1 Then
...
If FX Then
    ...
    Call SendData(SendTarget.ToNPCArea, nIndex, Map, "CXF" & Npclist(nIndex).char.CharIndex & "," & FXIDs.FXWARP & "," & 0)
End If
...
If MiNPC.flags.AguaValida = 1 Then
    Call CrearNPC2(MiNPC.Numero, MiNPC.Pos.Map, MiNPC.Orig)
    Exit Sub
End If
...
'CHOTS | Modificado, divide oro en party
'CHOTS | Modificado, multiplica oro

Dim gold  As Long
gold = MiNPC.GiveGLD
If MultOro <> 0 Then gold = gold * MultOro

If MiNPC.GiveGLD > 0 And UserIndex <> 0 Then
    If UserList(UserIndex).PartyIndex > 0 Then
        Call mdParty.DividirOro(UserIndex, gold, UserList(UserIndex).Pos.Map, UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y)
    Else
        UserList(UserIndex).Stats.GLD = UserList(UserIndex).Stats.GLD + gold
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "CHO" & gold)
    End If
End If
...
'CHOTS | Monturas
Npclist(NpcIndex).probCaptura = val(Leer.GetValue("NPC" & NpcNumber, "Probcaptura"))

'CHOTS | Guerras
Npclist(NpcIndex).salaGuerra = val(Leer.GetValue("NPC" & NpcNumber, "SalaGuerra"))
...
'CHOTS | Guerras
Npclist(NpcIndex).guerra.team = 0
