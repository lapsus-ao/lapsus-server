...
'    Call SendData(SendTarget.ToNPCArea, NpcIndex, Npclist(NpcIndex).Pos.Map, ServerPackages.dialogo & vbGreen & "° Soy Pretoriano °" & Str(ind))
...
On Error GoTo errorh
    ...
    If TeleFrag > 0 Then
        ...
        If (LegalPos(wp2.Map, wp2.X, wp2.Y)) Then
            ''mover al actual
            Call SendData(SendTarget.ToMap, 0, wp2.Map, ServerPackages.moverPersonaje & Npclist(TeleFrag).char.CharIndex & "," & wp2.X & "," & wp2.Y)
...
On Error GoTo errorh
    ...
    Else
        If Not (Npclist(npcind).Invent.BarcoSlot = 6) Then
            ...
            Call SendData(SendTarget.ToMap, npcind, Npclist(npcind).Pos.Map, "CXF" & Npclist(npcind).char.CharIndex & "," & 0 & "," & 0)
        End If
...
'        azar = 1
    ...
    Case 2          ''apocalipsis Rahma Nañarak O'al
        Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, ServerPackages.dialogo & vbCyan & "°" & Hechizos(Npclist(npcind).Spells(DAT_APOCALIPSIS)).PalabrasMagicas & "°" & str(Npclist(npcind).char.CharIndex))
        Call NpcLanzaSpellSobreUser2(npcind, BestTarget, Npclist(npcind).Spells(DAT_APOCALIPSIS)) ''SPELL 1 de Mago: Apocalipsis
    Case 3
    
        Call SendData(SendTarget.ToMap, npcind, Npclist(npcind).Pos.Map, "CXF" & Npclist(npcind).char.CharIndex & "," & FXIDs.FXMEDITAR54 & "," & LoopAdEternum)
...
'                            GreedyWalkTo npcind, NPCPosM, Npclist(NPCAlInd).Pos.X, Npclist(NPCAlInd).Pos.Y
    ...
    Case 1  '' paralizar enemigo PJ
        Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, ServerPackages.dialogo & vbCyan & "°" & Hechizos(Npclist(npcind).Spells(DAT_PARALIZARPJ)).PalabrasMagicas & "°" & str(Npclist(npcind).char.CharIndex))
    ...
    Case 2  '' ataque a usuarios (invisibles tambien)
        Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, ServerPackages.dialogo & vbCyan & "°" & Hechizos(Npclist(npcind).Spells(DAT_TORMENTAAVANZADA)).PalabrasMagicas & "°" & str(Npclist(npcind).char.CharIndex))
...
On Error GoTo errorh
    ...
    Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, ServerPackages.dialogo & vbCyan & "° " & Hechizos(indireccion).PalabrasMagicas & " °" & str(Npclist(npcind).char.CharIndex))
    Call SendData(SendTarget.ToNPCArea, NPCAlInd, Npclist(NPCAlInd).Pos.Map, "CXF" & Npclist(NPCAlInd).char.CharIndex & "," & Hechizos(indireccion).FXgrh & "," & Hechizos(indireccion).loops)
...
On Error GoTo errorh
    ...
    '' Envia las palabras magicas, fx y wav del indice-esimo hechizo del npc-hostiles.dat
    Call SendData(SendTarget.ToNPCArea, paralizador, Npclist(paralizador).Pos.Map, ServerPackages.dialogo & vbCyan & "° " & Hechizos(indireccion).PalabrasMagicas & " °" & str(Npclist(paralizador).char.CharIndex))
    Call SendData(SendTarget.ToNPCArea, paralizador, Npclist(paralizador).Pos.Map, "TW" & Hechizos(indireccion).WAV)
    Call SendData(SendTarget.ToNPCArea, Paralizado, Npclist(Paralizado).Pos.Map, "CXF" & Npclist(Paralizado).char.CharIndex & "," & Hechizos(indireccion).FXgrh & "," & Hechizos(indireccion).loops)
    'Call SendData(SendTarget.ToNPCArea, paralizador, Npclist(paralizador).Pos.Map, ServerPackages.dialogo & vbCyan & "° HOAX MANTRA °" & Str(Npclist(paralizador).Char.CharIndex))
    'Call SendData(SendTarget.ToNPCArea, paralizador, Npclist(paralizador).Pos.Map, "TW" & Hechizos(Npclist(paralizador).Spells(1)).WAV)
    'Call SendData(SendTarget.ToNPCArea, Paralizado, Npclist(Paralizado).Pos.Map, "CXF" & Npclist(Paralizado).Char.CharIndex & "," & Hechizos(Npclist(paralizador).Spells(1)).FXgrh & "," & Hechizos(Npclist(paralizador).Spells(1)).loops)
...
On Error GoTo errorh
    ...
    '' Envia las palabras magicas, fx y wav del indice-esimo hechizo del npc-hostiles.dat
    Call SendData(SendTarget.ToNPCArea, curador, Npclist(curador).Pos.Map, ServerPackages.dialogo & vbCyan & "° " & Hechizos(indireccion).PalabrasMagicas & " °" & str(Npclist(curador).char.CharIndex))
    Call SendData(SendTarget.ToNPCArea, curador, Npclist(curador).Pos.Map, "TW" & Hechizos(indireccion).WAV)
    Call SendData(SendTarget.ToNPCArea, curado, Npclist(curado).Pos.Map, "CXF" & Npclist(curado).char.CharIndex & "," & Hechizos(indireccion).FXgrh & "," & Hechizos(indireccion).loops)
    'Call SendData(SendTarget.ToNPCArea, curador, Npclist(curador).Pos.Map, ServerPackages.dialogo & vbCyan & "°EN CORP SANCTIS°" & Str(Npclist(curador).Char.CharIndex))
    'Call SendData(SendTarget.ToNPCArea, curador, Npclist(curador).Pos.Map, "TW" & Hechizos(Npclist(curador).Spells(3)).WAV)
    'Call SendData(SendTarget.ToNPCArea, curado, Npclist(curado).Pos.Map, "CXF" & Npclist(curado).Char.CharIndex & "," & Hechizos(Npclist(curador).Spells(3)).FXgrh & "," & Hechizos(Npclist(curador).Spells(3)).loops)
...
On Error GoTo errorh
    ...
    Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, ServerPackages.dialogo & vbCyan & "° " & Hechizos(indireccion).PalabrasMagicas & " °" & str(Npclist(npcind).char.CharIndex))
    Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, "TW" & Hechizos(indireccion).WAV)
    Call SendData(SendTarget.ToPCArea, PJEnInd, UserList(PJEnInd).Pos.Map, "CXF" & UserList(PJEnInd).char.CharIndex & "," & Hechizos(indireccion).FXgrh & "," & Hechizos(indireccion).loops)
    'Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, ServerPackages.dialogo & vbCyan & "°CAE XITÄ°" & Str(Npclist(npcind).Char.CharIndex))
    ...
    If Npclist(npcind).Name = "Rey Pretoriano" Then
        Call SendData(SendTarget.ToIndex, PJEnInd, 0, ServerPackages.dialogo & "El rey pretoriano te ha vuelto ciego " & FONTTYPE_FIGHT)
        Call SendData(SendTarget.ToIndex, PJEnInd, 0, ServerPackages.dialogo & "A la distancia escuchas las siguientes palabras: ¡Cobarde, no eres digno de luchar conmigo si escapas! " & FONTTYPE_VENENO)
    End If
...
On Error GoTo errorh
    ...
    Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, "TW" & Hechizos(indireccion).WAV)
    Call SendData(SendTarget.ToPCArea, PJEnInd, UserList(PJEnInd).Pos.Map, "CXF" & UserList(PJEnInd).char.CharIndex & "," & Hechizos(indireccion).FXgrh & "," & Hechizos(indireccion).loops)
    'Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, ServerPackages.dialogo & vbCyan & "°ASYNC GAM ALÚ°" & Str(Npclist(npcind).Char.CharIndex))
    'Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, "TW" & Hechizos(3).WAV)
    'Call SendData(SendTarget.ToNPCArea, curado, Npclist(curado).Pos.Map, "CXF" & Npclist(curado).Char.CharIndex & "," & Hechizos(Npclist(curador).Spells(3)).FXgrh & "," & Hechizos(Npclist(curador).Spells(3)).loops)
...
#End If
    'bardea si es el rey
    If Npclist(npcind).Name = "Rey Pretoriano" Then
        Call SendData(SendTarget.ToIndex, PJEnInd, 0, ServerPackages.dialogo & "El rey pretoriano te ha vuelto estúpido " & FONTTYPE_FIGHT)
    End If
Exit Sub
...
On Error GoTo errorh
    ...
    Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, ServerPackages.dialogo & vbCyan & "° " & Hechizos(indireccion).PalabrasMagicas & " °" & str(Npclist(npcind).char.CharIndex))
    Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, "TW" & Hechizos(indireccion).WAV)
    Call SendData(SendTarget.ToPCArea, PJEnInd, UserList(PJEnInd).Pos.Map, "CXF" & UserList(PJEnInd).char.CharIndex & "," & Hechizos(indireccion).FXgrh & "," & Hechizos(indireccion).loops + 4)

    'Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, ServerPackages.dialogo & vbCyan & "°AN ROHL ÙX MAÏO°" & Str(Npclist(npcind).Char.CharIndex))
    'Call SendData(SendTarget.ToPCArea, PJEnInd, UserList(PJEnInd).Pos.Map, "CXF" & UserList(PJEnInd).Char.CharIndex & "," & Hechizos(4).FXgrh & "," & Hechizos(4).loops + 4)
    'Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, "TW" & Hechizos(4).WAV)
    
    Call SendData(SendTarget.ToIndex, PJEnInd, 0, ServerPackages.dialogo & "Comienzas a hacerte visible." & FONTTYPE_VENENO)
    ...
    'Call SendData(SendTarget.ToMap, 0, UserList(PJEnInd).Pos.Map, nover(5) & UserList(PJEnInd).Char.CharIndex & ",0")
Exit Sub
...
On Error GoTo errorh
    ...
    Select Case bs
        Case 5
            Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, ServerPackages.dialogo & vbGreen & "°Rahma°" & str(Npclist(npcind).char.CharIndex))
            Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, "TW" & SONIDO_DRAGON_VIVO)
        Case 4
            Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, ServerPackages.dialogo & vbGreen & "°vôrtax °" & str(Npclist(npcind).char.CharIndex))
            Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, "TW" & SONIDO_DRAGON_VIVO)
        Case 3
            Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, ServerPackages.dialogo & vbGreen & "°Zill°" & str(Npclist(npcind).char.CharIndex))
            Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, "TW" & SONIDO_DRAGON_VIVO)
        Case 2
            Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, ServerPackages.dialogo & vbGreen & "°yäkà E'nta°" & str(Npclist(npcind).char.CharIndex))
            Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, "TW" & SONIDO_DRAGON_VIVO)
        Case 1
            Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, ServerPackages.dialogo & vbGreen & "°¡¡Koràtá!!°" & str(Npclist(npcind).char.CharIndex))
            Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, "TW" & SONIDO_DRAGON_VIVO)
        Case 0
            Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, ServerPackages.dialogo & vbGreen & "° °" & str(Npclist(npcind).char.CharIndex))
            ...
            For X = 8 To 95
                For Y = 8 To 95
                    ...
                    If PJInd > 0 Then
                        ...
                        Call SendData(SendTarget.ToIndex, PJInd, 0, ServerPackages.dialogo & Npclist(npcind).Name & " te ha quitado " & danioI & " puntos de vida al romper su vara." & FONTTYPE_FIGHT)
                        Call SendData(SendTarget.ToPCArea, PJInd, UserList(PJInd).Pos.Map, "TW" & Hechizos(indireccion).WAV)
                        Call SendData(SendTarget.ToPCArea, PJInd, UserList(PJInd).Pos.Map, "CXF" & UserList(PJInd).char.CharIndex & "," & Hechizos(indireccion).FXgrh & "," & Hechizos(indireccion).loops)
                    ...
                    ElseIf (MascotaInd > 0) Then
                        If (Npclist(MascotaInd).MaestroUser > 0) Then
                            ...
                            Call SendData(SendTarget.ToNPCArea, MascotaInd, Npclist(MascotaInd).Pos.Map, "CXF" & Npclist(MascotaInd).char.CharIndex & "," & Hechizos(indireccion).FXgrh & "," & Hechizos(indireccion).loops)
...
If (NPCx = X And NPCy = Y) Then Exit Sub
''ya llegué!!
        ...
        Else    ''x completitud, esta en la misma Y
            ...
            Else
                ...
                If Npclist(npcorig).CanAttack = 1 And (RandomNumber(1, 100) > 95) Then
                    Call SendData(SendTarget.ToNPCArea, npcorig, Npclist(npcorig).Pos.Map, ServerPackages.dialogo & vbYellow & "°Maldito bastardo, ¡ ven aquí !°" & str(Npclist(npcorig).char.CharIndex))
...
On Error GoTo errorh
    ...
    Call SendData(SendTarget.ToMap, 0, Npclist(npcorig).Pos.Map, ServerPackages.moverPersonaje & Npclist(npcorig).char.CharIndex & "," & NPCx & "," & NPCy + 1)
...
On Error GoTo errorh
    ...
    Call SendData(SendTarget.ToMap, 0, Npclist(npcorig).Pos.Map, ServerPackages.moverPersonaje & Npclist(npcorig).char.CharIndex & "," & NPCx - 1 & "," & NPCy)
    'Update map and npc pos
    MapData(mapa, NPCx, NPCy).NpcIndex = 0
...
''On Error Resume Next
    ...
    If (Npclist(npcind).Pos.Map = MAPA_PRETORIANO) And (UserList(PJEnInd).Pos.Map = MAPA_PRETORIANO) Then
        ...
        '    If Npclist(npcind).CanAttack = 1 Then
        '        Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, ServerPackages.dialogo & vbYellow & "°¡ Cobarde !°" & str(Npclist(npcind).Char.CharIndex))
        '        Npclist(npcind).CanAttack = 0
        '    End If
        'End If
    Else
...
On Error GoTo errorh
    ...
    If LegalPos(NPCPosM, NPCPosX + 1, NPCPosY + 1) Then
        Call SendData(SendTarget.ToMap, 0, NPCPosM, ServerPackages.moverPersonaje & Npclist(npcind).char.CharIndex & "," & NPCPosX + 1 & "," & NPCPosY + 1)
        'Update map and npc pos
        ...
        Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, ServerPackages.dialogo & vbYellow & "°¡¡JA JA JA JA!!°" & str(Npclist(npcind).char.CharIndex))
        Exit Sub
    End If
    ...
    If Npclist(npcind).CanAttack = 1 Then
        Call SendData(SendTarget.ToNPCArea, npcind, Npclist(npcind).Pos.Map, ServerPackages.dialogo & vbYellow & "°¡Por las barbas de los antiguos reyes! ¡Alejáos endemoniados espectros o sufriréis la furia de los dioses!°" & str(Npclist(npcind).char.CharIndex))
        Npclist(npcind).CanAttack = 0
    End If
