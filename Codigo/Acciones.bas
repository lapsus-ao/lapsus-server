...
If Not (Distance(UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y, X, Y) > 2) Then
    If ObjData(MapData(Map, X, Y).OBJInfo.ObjIndex).Llave = 0 Then
        If ObjData(MapData(Map, X, Y).OBJInfo.ObjIndex).Cerrada = 1 Then
                ...
                Else
                     Call SendData(SendTarget.ToIndex, UserIndex, 0, ServerPackages.dialogo & "La puerta esta cerrada con llave." & FONTTYPE_INFO)
                End If
        Else
                'Cierra puerta
...
If MapData(Map, X, Y).trigger = eTrigger.ZONASEGURA Or MapInfo(Map).Pk = False Then
    Call SendData(SendTarget.ToIndex, UserIndex, 0, ServerPackages.dialogo & "En zona segura no puedes hacer fogatas." & FONTTYPE_INFO)
    Exit Sub
End If
...
If exito = 1 Then
    If MapInfo(UserList(UserIndex).Pos.Map).zona <> Ciudad Then
        ...
        Call SendData(ToIndex, UserIndex, 0, ServerPackages.dialogo & "Has prendido la fogata." & FONTTYPE_INFO)
    ...
    Else
        Call SendData(ToIndex, UserIndex, 0, ServerPackages.dialogo & "La ley impide realizar fogatas en las ciudades." & FONTTYPE_INFO)
        Exit Sub
    End If
Else
    Call SendData(ToIndex, UserIndex, 0, ServerPackages.dialogo & "No has podido hacer fuego." & FONTTYPE_INFO)
End If
