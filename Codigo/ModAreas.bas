...
'**************************************************************
    ...
    With UserList(UserIndex)
        ...
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "CÑ" & Chr$(.Pos.X) & Chr$(.Pos.Y))
...
#If SeguridadAlkon Then
                        If EncriptarProtocolosCriticos Then
                            If UserList(TempInt).flags.Invisible Or UserList(TempInt).flags.Oculto Then
                                 Call EnviarDatosASlot(UserIndex, ProtoCrypt(Nover(5) & UserList(TempInt).char.CharIndex & ",1", UserIndex) & ENDC)
                            End If
                            
                            If UserList(UserIndex).flags.Invisible Or UserList(UserIndex).flags.Oculto Then
                                 Call EnviarDatosASlot(TempInt, ProtoCrypt(Nover(5) & UserList(UserIndex).char.CharIndex & ",1", TempInt) & ENDC)
                            End If
                        Else
#End If
                            If UserList(TempInt).flags.Invisible Or UserList(TempInt).flags.Oculto Then
                                Dim ChotsNover As String
                                ChotsNover = UserList(TempInt).char.CharIndex & ",1" & ENDC
                                'ChotsNover = Encriptar(ChotsNover)
                                Call EnviarDatosASlot(UserIndex, Nover(5) & ChotsNover)
                            End If
                            
                            If UserList(UserIndex).flags.Invisible Or UserList(UserIndex).flags.Oculto Then
                                'Dim ChotsNover As String
                                ChotsNover = UserList(UserIndex).char.CharIndex & ",1" & ENDC
                                'ChotsNover = Encriptar(ChotsNover)
                                Call EnviarDatosASlot(TempInt, Nover(5) & ChotsNover)
                            End If
#If SeguridadAlkon Then
                        End If
#End If
